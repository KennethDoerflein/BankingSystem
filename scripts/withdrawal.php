...
try {
    // Get posted data
    $withdrawal = floatval(trim($_POST['withdrawal']));
  ...
  $stmt = $db->prepare("INSERT INTO transactions (transactionID, dateOfTransaction, transactionType, changeInBalance, bankAccountNumber) VALUES (?, CURRENT_TIMESTAMP, ?, ?, ?)");
    $type = "withdrawal";
    $negativeWithdrawal = -$withdrawal; // Make withdrawal amount negative
  $stmt->bind_param("ssds", $transactionID, $type, $negativeWithdrawal, $acctNum);
    $stmt->execute();
